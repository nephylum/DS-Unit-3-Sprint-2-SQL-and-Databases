...
- In the Northwind database, what is the type of relationship between the
  ...
  The relationship could be described as many to many. There are many Employees
  and territories and each employee may have more than one territory assigned
  them.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

  MongoDB is a NoSQL database, it stores documents inside a collection. It is
  not a relational database, and would be appropriate for high-performance,
  dynamic data storage. As it's not a relational database it would be unwieldy
  to use for relational databases (schema's that build tables) and have many
  transactions (changes, additions), it is also not ACID compliant.

- What is "NewSQL", and what is it trying to achieve?
NewSQL is a system that aims to provide the same performance and scalability of
NoSQL systems while keeping adherence to ACID. It is basically trying to have
the best of both relational databases (high transactions) and non-relational
databases (size and performance).
